...
class Discovery extends StatefulWidget {
  ...
  const Discovery({Key? key, this.category, this.location, this.feature})
      : super(key: key);
...
class _DiscoveryState extends State<Discovery> {
  ...
  void initState() {
    ...
    if (Application.submitSetting.locations
            .where((e) => e.locationType == LocationType.country)
            .length ==
        1) {
      _filter.country = Application.submitSetting.locations
          .singleWhere((e) => e.locationType == LocationType.country);
    }
    ...
    if (widget.location != null &&
      ...
      _filter.country = Application.submitSetting.locations
          .singleWhere((e) => e.id == widget.location!.parentId);
    ...
    _reviewSubscription = AppBloc.reviewCubit.stream.listen((state) {
      if (state is ReviewSuccess && state.postId != null) {
        AppBloc.discoveryCubit.onUpdate(state.postId!);
      }
    });
    // _onRefresh();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (context) => AppBloc.discoveryCubit,
      child: Scaffold(
        appBar: AppBar(
          centerTitle: true,
