...
class _FilterState extends State<Filter> {
  ...
  void initState() {
    ...
    _filter.country = Application.currentCountry;
  ...
  void _onChangeState() async {
    ...
    _loadingState = true;
    setState(() {});
    var list = await LocationRepository.loadLocationById(
    ...
    final selected = await Navigator.pushNamed(
      ...
      arguments: PickerModel(
        title: Translate.of(context).translate('select_state'),
  ...
  void _onChangeCity() async {
    _listCity = [
      LocationModel(
          id: 0,
          parentId: 0,
          title: Translate.of(context).translate("all_city"),
          locationType: LocationType.city)
    ];
    _loadingCity = true;
    setState(() {});
    var list = await LocationRepository.loadLocationById(_filter.state!.id);
    ...
    final selected = await Navigator.pushNamed(
      ...
      Routes.picker,
      arguments: PickerModel(
        title: Translate.of(context).translate('select_city'),
  ...
  void _onChangeCurrency() async {
    final selected = await Navigator.pushNamed(
      ...
      Routes.picker,
      arguments: PickerModel(
        title: Translate.of(context).translate('select_currency'),
  ...
  List<Widget> _onCreateField() {
    ...
    _filter.category?.groups?.forEach((group) {
      group.properties
          ?.where((property) => property.propertyType == PropertyType.filter)
          .forEach((item) {
      ...
      chips.add(
        SizedBox(
          ...
          child: FilterChip(
            selected: e.value ==
                _filter.extendedAttributes![index].getValue().toString(),
            label: Text(Translate.of(context).translate(e.title)),
            onSelected: (check) {
              if (check) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Visibility(
              visible: _filter.state != null || _filter.city != null,
              child: Column(
                children: [
                  const SizedBox(height: 16),
                  InkWell(
            ...
            Wrap(
              ...
              children: Application.setting.sortOptions.map((item) {
                ...
                return SizedBox(
                  ...
                  child: FilterChip(
                    selected: selected,
                    label: Text(Translate.of(context).translate(item.title)),
                    onSelected: (check) {
                      if (check) {
