...
class _ProfileState extends State<Profile> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _reviewSubscription = AppBloc.reviewCubit.stream.listen((state) {
      if (state is ReviewSuccess && state.postId != null) {
        _onRefresh();
      }
    });
    _onRefresh();
  ...
  Future<void> _onAction(CommentModel item, BuildContext context) async {
                ...
                child: Column(
                  children: [
                    ...
                    Column(
                      children: [
                        bookingItem,
                        if (AppBloc.userCubit.state!.id == widget.user.id &&
                            item.reviewType == ReviewType.review &&
                            (item.replaies != null &&
                                !item.replaies!.any((element) =>
                                    element.createdBy.id ==
                        ...
                        if (AppBloc.userCubit.state!.id == item.createdBy.id)
                          AppListTitle(
                            title: Translate.of(context).translate("edit"),
                            leading: const Icon(Icons.share_outlined),
                            onPressed: () {
    ...
    if (result == 'replay') {
      List<Object> obj = [item.postId, item, "replay"];
    ...
    if (result == 'edit') {
      ...
      List<Object> obj = [item.postId, item, "edit"];
      await Navigator.pushNamed(
        context,
        Routes.writeReview,
        arguments: obj,
    ...
    if (result == 'remove') {
      await AppBloc.reviewCubit.onRemove(id: item.id, type: item.reviewType);
      await _onRefresh();
    }
  }
  ...
  void _onProductDetail(ProductModel item) {
    if (item.createdBy != null &&
        item.createdBy!.id == AppBloc.userCubit.state?.id) {
  ...
  Widget _buildContent({
            ...
            (context, index) {
              ...
              return Padding(
                ...
                child: AppProductItem(
                  item: item,
                  type: AppBloc.userCubit.state?.id == user?.id
    ...
    if (widget.user.id == AppBloc.userCubit.state!.id &&
    ...
    ///Build List Comment
    if (((widget.user.id != AppBloc.userCubit.state!.id && _indexTab == 1) ||
            _indexTab == 2) &&
        listComment != null) {
